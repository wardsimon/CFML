...
!!
 Module CFML_Optimization_General
    ...
    !---- store the different methods available in the module ----!
 ...
 Contains
    ...
    Subroutine Init_Opt_Conditions(Opt)
       ...
       Opt=Opt_Conditions_Type("Simplex",      &   ! Name of Method
                                0,             &   ! Code for Method
                                0,             &   ! Number of Free Parameters
                                1000,          &   ! Maximum number of function calls
                                2000,          &   ! Loops (Simplex Method) fitting a quadratic
                                1,             &   ! /=0  fitting a quadratic model (Simple Method)
                                0,             &   ! Printer parameter
                                0,             &   ! Flag value state on exit subroutine
                                0,             &   ! Total number of Functions / Gradient evaluations
                                0,             &   ! Total number of search directions used
                                1.0e-6_cp,     &   ! Convergence parameter
                                1.0e-20_cp)        ! Estimate machine accuracy
    ...
    !!--++
    !!--++ Subroutine Local_DFP (N,Eps,Maxfn,X,F,Nfev,Mmin,Mmax, Model_Functn)
    !!--++
    ...
    Subroutine Local_DFP(N, Eps, Maxfn, X, F, Nfev, Mmin, Mmax, Model_Functn)
       ...
       real(kind=cp),               intent(in)      :: eps         ! Convergence Criterion
       integer,                     intent(in)      :: maxfn       ! Maximum Number Of Function Evaluations
       real(kind=cp), dimension(:), intent(in out)  :: x           ! Vector Of Length N Containing Parameter Values
       ...
       real(kind=cp), dimension(:), intent(in)      :: mmin        ! Lower bounds of the parameters
       real(kind=cp), dimension(:), intent(in)      :: mmax        ! Upper bounds of the parameters
    ...
    Subroutine Local_Min_DFP(Model_Functn, N, X, F, C, mini, maxi, Ipr)
       ...
       real(kind=cp), dimension(:), intent(in out)  :: x         ! Vector Of Length N Containing Parameter Values
                                                                 ...
                                                                 ! Out-> The Final Parameter Estimates As Determined By Local
       real(kind=cp),               intent(out)     :: f         ! The Value Of The Function At The Final Parameter Estimates
       type(Opt_conditions_Type),   intent(in out)  :: C         ! Lower bounds of the parameters
       real(kind=cp), dimension(:), intent(in)      :: mini      ! Upper bounds of the parameters
       real(kind=cp), dimension(:), intent(in)      :: maxi      ! Conditions for the algorithm. C is of type(Opt_conditions)
    ...
    Subroutine Local_Optimize(Model_Functn,X,F,C,G,mini,maxi,v,Ipr)
       ...
       type(Opt_conditions_type),            intent(in out) :: C     ! Conditions for the algorithm. C is of type(Opt_conditions)
       ...
       real(kind=cp),dimension(:), optional, intent(in out) :: maxi  !
    ...
    Subroutine Local_Rand(N,Relcon,Maxfn,X,F,Nfev,Mini,Maxi,Model_Functn)
       ...
       real(kind=cp),               intent(in)     :: relcon      ! convergence criterion.
       integer,                     intent(in)     :: maxfn       ! maximum number of function evaluations allowed
       real(kind=cp), dimension(:), intent(in out) :: x           ! Normalized vector of dimension n containing parameter values
       real(kind=cp),               intent(in out) :: f           ! value at the final parameter estimates
       integer,                     intent(out)    :: nfev        ! number of function evaluations
       real(kind=cp), dimension(:), intent(in)     :: mini        ! scaling factors supplied by the global routine
       real(kind=cp), dimension(:), intent(in)     :: maxi        ! scaling factors supplied by the global routine
       Interface
          Subroutine Model_Functn(n,x,f,g)
             use CFML_GlobalDeps,  only: cp
             integer,                             intent(in) :: n
    ...
    !!----
    !!---- Subroutine Nelder_Mead_Simplex(Model_Functn, Npar, P, Step, Var, Func, C, Ipr)
    !!----
    !!--<<    Interface
    ...
    Subroutine Nelder_Mead_Simplex(Model_Functn, Nop, P, Step, Var, Func, C, Ipr)
       ...
       real(kind=cp),                intent(out)     :: func      ! Out-> the function value corresponding to the final
                                                                  !       parameter values.
       type(opt_conditions_Type),    intent(in out)  :: c         ! Optimization Conditions
       integer, optional,            intent(in)      :: Ipr       ! Logical unit for printing if the parameter C%IOUT /= 0.
    ...
    !!--++ Update: February - 2005
    !!
    Subroutine Print_Tri_Matrix(Array, N, Iunit)
       !---- Arguments ----!
       real(kind=cp),dimension(:),  intent(in)  :: array   ! Array
       integer,                     intent(in)  :: n       ! Order of array
       integer,                     intent(in)  :: iunit   ! Ouput unit
       ...
       do l = 1, n, 6
          ...
          do i = l, n
             ...
             ii = ii + i
             i2 = min(ii,i1+5)
             write(unit=iunit,fmt="(tr1,6f13.5)") array(i1:i2)
          end do
       end do
    ...
    Subroutine Syminv(A, N, C, Nullty, Ifault, Rmax)
       !---- Arguments ----!
       real(kind=cp), dimension(:),  intent(in)     :: A       ! Symmetric matrix to be inverted, stored in lower triangular form
       integer,                      intent(in)     :: N       ! Order of A
       real(kind=cp), dimension(:),  intent(out)    :: C       ! Inverse of A (A generalized inverse if C is singular), also stored in lower triangular form.
       integer,                      intent(out)    :: Nullty  ! the rank deficiency of A.
       integer,                      intent(out)    :: Ifault  ! Error Code: 1 if N < 1, 2 If A Is not +ve semi-definite, 0 otherwise
