...
!(the operation {1|0}' is present) the primed operation are put at the bottom.
      ...
      if( m /= Multip) then !Check that it is OK
        write(unit=Err_CFML%Msg,fmt="(2(a,i4))") " Warning in Reorder_Operators@CFML_Rational_Groups! Multip=",Multip, " Calculated Multip: ",m
        Err_CFML%Ierr = 1
        return
      end if
      ...
      if(ng /= Multip) then
        Err_CFML%Ierr = 1
        write(unit=Err_CFML%Msg,fmt="(2(a,i3))") " => Problem in Reorder_Operators@CFML_Rational_Groups:! the multiplicity ",Multip," has not been recovered, value of ng=",ng
      end if
