  Module DataRed_Treat_Reflections_Mod
   ...
   contains
    ...
    Subroutine Treat_Reflections_nonConv(mode,R,cond,cell,SpG,kinfo,tw,lun)
      ...
      character(len=*),parameter,dimension(0:1) :: warn_mess=["                      ", " <- Dubious reflection"]
      character(len=*),parameter,dimension(0:2) :: inf=["Nuclear  Reflection", "Magnetic Reflection","Mixed    Reflection"]
      ...
      do i=1,R%nref       !Loop over all reflections
         ...
         if (itreat(i) == 0) then   !If not yet treated do the following
            ...
            write(unit=iou,fmt=fm1) i,hh,"                  ",R%Ref(i)%intens,R%Ref(i)%sigma,R%Ref(i)%numor,R%Ref(i)%twtheta
            ...
            write(unit=iou,fmt=fm2)"   =>   ",h_rep(:,indp),nt(indp),ini(indp),fin(indp),intav(indp),sigmav(indp),warn_mess(warn(indp))//"    "//inf(info(4))
            write(unit=ihkl,fmt=fm3) h_rep(:,indp),intav(indp),sigmav(indp),abs(R%Ref(i)%idomain), R%Ref(i)%twtheta, 0.0, 0.0, 0.0,warn_mess(warn(indp))//"    "//inf(info(4))
