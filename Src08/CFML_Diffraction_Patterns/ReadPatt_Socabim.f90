SubModule (CFML_Diffraction_Patterns) ReadPatt_Socabim
    ...
    Module Subroutine Read_Pattern_Socabim(Filename,Pat)
       ...
       DO
           ...
           if (ier /= 0) then
              Err_CFML%state=.true.
              Err_CFML%Flag=2
              Err_CFML%Msg=" Error on Socabim UXD Intensity file, check your mode parameter!"
        ...
        if (pat%npts <= 0) then
           ...
           if (step > 0.000001) then
              pat%npts= nint((160.0-pat%xmin)/step + 1.0)
           else
              Err_CFML%state=.true.
              Err_CFML%Flag=2
        ...
        do    
           ...
           if (ier /= 0) then
              Err_CFML%state=.true.
              Err_CFML%Flag=2
              Err_CFML%Msg=" Error on Socabim UXD Intensity file, check your instr parameter!"
...
End SubModule ReadPatt_Socabim