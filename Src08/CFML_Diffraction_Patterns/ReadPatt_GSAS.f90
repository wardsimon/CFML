SubModule (CFML_Diffraction_Patterns) ReadPat_GSAS
    ...
    Module Subroutine Read_Pattern_Gsas(Filename, Pat, mode)
       ...
       if (.not. keep_open) then
          ...
          if (bank_missed) then
             Err_CFML%state=.true.
             Err_CFML%Flag=2
             Err_CFML%Msg=" => Error in the input GSAS-file: BANK not found!"
       ...
       look_bank: do
          ...
          if ( ibank /= nbank) then  !Verify that we have the proper bank
             do
                ...
                if (ier /= 0) then
                   Err_CFML%state=.true.
                   Err_CFML%Flag=2
                   write(unit=Err_CFML%Msg,fmt="(a,i2,a)") " Error in Intensity file, BANK number: ",nbank," not found!"
          ...
          if (datyp == "STD") then
             ...
             if (bintyp == "CONST") then
                ...
                Err_CFML%state=.true.
                Err_CFML%Flag=2
                Err_CFML%Msg=" => Only BINTYP=CONST is allowed for ESD data" 
             ...
             if (bintyp == "RALF") then
                ...
                Err_CFML%state=.true.
                Err_CFML%Flag=2
                Err_CFML%Msg=" =>  Only BINTYP=RALF or CONST is allowed for ALT data"
