...
!!
 Module CFML_String_Utilities
    ...
    public :: Cutst, Format_Real, Frac_Trans_1Dig, Frac_Trans_2Dig, Get_DateTime, Get_Dirname, &
              ...
              Get_Transf, Get_Vec_From_String, Getnum, Getnum_std, Getword, Inc_LineNum,       &
              Init_FindFmt, Lcase, Number_lines, NumCol_from_NumFmt, Read_Key_Str,             &
              Read_Key_strVal, Read_Key_Value, Read_Key_ValueSTD, Reading_Lines, Setnum_std,   &
    ...
    !> Variables
    Integer, private, save       :: Line_Nb       ! Line number (findFMT)
 ...
 Contains
    ...
    !!
    Pure Function Equal_Sets_Text(str1,n1,str2,n2) result(Equal_sets_texto)
       !---- Arguments ----!
   ...
   !!----      -2 : FORTRAN read error
    ...
    !!---- Update: January - 2009
    !!
    Subroutine FindFmt(IUnit,aLine,FMTfields,FMTstring,idebug)
       !---- Arguments ----!
    ...
    Subroutine Format_Real(Val,W,fmtcar) 
       ...
       if (x > xlim)then         ! need to write in e format
          ...
          write(unit=fmtcar,fmt=trim(forms)) val
          return
       end if
    ...
    !!
    Subroutine Get_Separator_Pos(Str,car,pos,ncar)
       !---- Arguments ----!
    ...
    !!----
    Subroutine Get_Transf(str,mat,v,cod)
       !---- Arguments ----!
       character(len=*),                          intent(in)  :: str      ! Input string
    ...
    Subroutine Getnum(Str,vet,ivet,iv)
       ...
       do
          ...
          if (.not. numero) then
             Err_CFML=.true.
             Err_CFML_Flag=2
             Err_CFML_Msg="The variable cannot be computed as a number in GETNUM "
             return
          end if
          ...
          do i=iniz,nchard
             ...
             if (idig >= 1 .and. idig <= 11)  then
                ...
                Err_CFML=.true.
                Err_CFML_Flag=2
                Err_CFML_Msg="Limits of digit variable exceeded in GETNUM"
                return
             end if
          end do
