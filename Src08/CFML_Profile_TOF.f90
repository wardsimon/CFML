...
Module CFML_PowderProfiles_TOF
    ...
    Use CFML_Math_General, only: erfc_deriv
 ...
 Contains
    ...
    Subroutine Tof_Carpenter(Dt,D,Alfa,Beta,Gamma,Eta,Kappa,Tof_Theta,Tof_Peak,Deriv)
       ...
       if(omg <= 1.0E-35) then
          ...
          erfpu=erfc_deriv(y_u)                         ! d(erfc(y_u))/dy_u
          erfpv=erfc_deriv(y_v)                         ! d(erfc(y_v))/dy_v
          erfps=erfc_deriv(y_s)                         ! d(erfc(y_s))/dy_s
          erfpr=erfc_deriv(y_r)                         ! d(erfc(y_r))/dy_r
          a_u=expu*erfpu
          a_v=expv*erfpv
          a_s=exps*erfps
          a_r=expr*erfpr
    ...
    Subroutine Tof_Jorgensen(Dt,Alfa,Beta,Sigma,Tof_Peak,Deriv)
       ...
       if (omeg <= 1.0e-30) then
          ...
          erfyp=erfc_deriv(y)         ! erfcc'(y)
          erfzp=erfc_deriv(z)         ! erfcc'(z)
          a=expu*erfyp
          b=expv*erfzp
          do i=1,udiv-1
