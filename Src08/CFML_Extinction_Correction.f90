...
Module CFML_Extinction_correction
   !---- Use Modules ----!
   use CFML_DefPar, only : CP, ERR_CFML, Err_CFML_Mess, Init_Err_CFML
 ...
 Contains
    !!----
    !!---- Function AG_theta(cos2t) result (ag)
    !!----
    ...
    !!----
    !!---- Function AL_theta(cos2t) result (ag)
    !!----
    !!---- Update: February 2017
    !!----
    !!
    ...
    !!----
    !!---- Function BG_theta(cos2t) result (ag)
    !!----
    !!---- Update: February 2017
    !!----
    !!
    ...
    !!----
    !!---- Function BL_theta(cos2t) result (ag)
    !!----
    !!---- Update: February 2017
    !!----
    !!
    ...
    Subroutine SHELX_Extinction(job,iext,Lambda,ssnn,hkl,f2,extc,ys,der,derf2)
       ...
       if (abs(qq) > 1.0_cp) then
          ERR_CFML=.true.
          write(ERR_CFML_mess,fmt="(a,3f8.2,a)") " ----> WARNING!: Extinction correction fixed to 1.0 for reflection: ",&
    ...
    Subroutine Correct_FlippingRatios(iext,Lambda,q,extc,ssnn,hkl,AN,BN,AM,BM,yp,ym,ypm,dyp,dym,dypm,dymag)
       ...
       !---- Local Variables ----!
       real(kind=cp), parameter    :: EPSIL=0.001_cp, CST=0.001_cp
