...
USE cryscalc_module,               ONLY : nb_help_max, nb_numor_max, help_string, HTML_unit, browse_cryscalc_HTML, my_browser, &
...
! --------------- setting file -----------------------------------
  ...
  WRITE(HTML_unit, '(2a)')'    address     = Centre de Diffractom&eacute;trie X, UMR6226 CNRS Universit&eacute; de Rennes 1,', &
                                            'Institut des Sciences Chimiques de Rennes, 35042 RENNES Cedex France'
  ...
  WRITE(HTML_unit, '(a)') '    structure_solution_name         = SHELXT'
  WRITE(HTML_unit, '(3a)')'    structure_solution_reference    = G.M. Sheldrick, Acta Cryst. A71 (2015) 3-8'
  WRITE(HTML_unit, '(a)') '    structure_solution_cif_ref      = SHELXT (Sheldrick, 2015)'
  WRITE(HTML_unit, '(a)') '    structure_refinement_name       = SHELXL-2014'
  WRITE(HTML_unit, '(a)') '    structure_refinement_reference  = G.M. Sheldrick, Acta Cryst. C71 (2015), 3-8'
  WRITE(HTML_unit, '(a)') '    structure_refinement_cif_ref    = SHELXL-2014/7 (Sheldrick, 2015)'

  WRITE(HTML_unit, '(a)') '    absorption_correction_name      = SADABS'
  WRITE(HTML_unit, '(2a)')'    absorption_correction_reference = Sheldrick G.M. (2014), SADABS Bruker AXS Inc., Madison, ', &
                                                                'Wisconsin, USA'
  WRITE(HTML_unit, '(2a)')'    absorption_correction_cif_ref   = Sheldrick G.M. (2014), SADABS Bruker AXS Inc., Madison, ', &
                                                                'Wisconsin, USA'
  ...
  WRITE(HTML_unit, '(a)') '    ref_overlap_criteria        = 0.10      ! criteria on hkl index for overlapped reflections criteria'
  WRITE(HTML_unit, '(a)') '    search_mono_criteria        = 2.5       ! max. diff. between monoclinic angle and 90. '
  WRITE(HTML_unit, '(a)') '    search_SG_only_mono         = 1         ! output only monoclinic space groups compatible with unit' &
                      //  ' cell metrics'

  WRITE(HTML_unit, '(a)') ''
  WRITE(HTML_unit, '(a)') '    <font color="darkred">[ARCHIVE_AND_REPORT]</font>'
  ...
  WRITE(HTML_unit, '(a)') '    CIF_author                  = 0         ! include author name and address in CIF file'
  ...
  WRITE(HTML_unit, '(a)') '    include_experimenter        = 1         ! include experimenter name in the archive_cryscalc.cif file'
  ...
  WRITE(HTML_unit, '(2a)') '    X_profile_U                  = 0.0055    ! U value of the Cagliotti formula : ', &
                                                                         ' cccFWHM2 = U*TAN**2(theta) + V*TAN(theta) + W'
  WRITE(HTML_unit, '(a)')  '    X_profile_V                  = -0.0015   ! V value '
  WRITE(HTML_unit, '(a)')  '    X_profile_W                  = 0.0036    ! W value '
  WRITE(HTML_unit, '(a)')  '    X_profile_eta0               = 0.3       ! Lorentzian components : eta = eta0 + 2theta * eta1'
  ...
  WRITE(HTML_unit, '(a)')  '    N_pattern_scale              = 100.       '
  ...
  if(nb_shortcuts /=0) then
    WRITE(HTML_unit, '(a)') '    <font color="darkred">[USER SHORTCUTS]</font>          ! only in expert mode'
    do i=1, nb_shortcuts
...
! ----------------------------------------------------------------
  ...
  WRITE(HTML_unit, '(2a)') ' <li class="item"><a href="http://www.cdifx.univ-rennes1.fr/progs/cryscalc/supercell_quartz.cfl">', &
                           '  Simulation of a X-ray diffraction pattern in a supercell (quartz example)</a><p>'
...
subroutine write_HTML_css(HTML_unit, input_string)
 ...
 implicit none
  ...
  WRITE(HTML_unit, '(2a)') "  .date    { font-family: 'Courier new';        font-size:14px; color:#660000;",   &
                           " font-weight:bold; margin-left:30px; background-color:#f5f5f5;}   "
  WRITE(HTML_unit, '(2a)') "  .founds   { color:#AAAAAA; font-size:12px;}"
