...
subroutine read_input_file_KEYWORDS()
 USE cryscalc_module, ONLY : input_unit, keyword_BEAM, keyword_SFAC_UNIT, keyword_CONT, keyword_CHEM, keyword_ZUNIT, &
                             debug_proc
...
!------------------------------
   ...
   CASE ('DIST', 'DISTANCE', 'ATOMIC_DISTANCE')
	...
	if(nb_arg /=0) then	 
	 do i=1, nb_arg
	  if(len_trim(arg_string(1)) > 4 .and.arg_string(i)(1:4) == 'MAX=') then
