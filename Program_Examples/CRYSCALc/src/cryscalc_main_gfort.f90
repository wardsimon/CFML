...
program crystallographic_calculations
 ...
 if (keyword_CLA) then
  ...
  OPEN(UNIT=2, FILE='CRYSCALC_cla.txt')
   ...
   call write_info('')
   call write_info('CRYSCALC command line arguments are stored in the CRYSCALC_cla.txt file.')
   call write_info('')
   stop
 endif
 ...
 if (keyword_create_CRYSCALC_NEWS) then
  close(unit=2)
  !OPEN(UNIT=2, FILE='cryscal_cnews.txt',   ACTION='write', STATUS='replace')
  OPEN(UNIT=2, FILE='cryscalc_news.txt')
  call write_CRYSCALC_title
