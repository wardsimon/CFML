...
program crystallographic_calculations
  ...
  if(nb_arg /=0) then
   ...
   do i = 1, nb_arg
    ...
    if(long >=4 .and. u_case(cmd_arg(i)(1:4)) == 'HKL=' ) then
	 ...
	 if(index(u_case(ABS_file_name), '.HKL') ==0) HKL_file_name = trim(HKL_file_name)//'.HKL' 
   ...
   if(i_arg_DEBUG /=0 .or. i_arg_NOOUT /=0 .or. i_arg_NOHKL /=0 .or. i_arg_CIFDEP /=0 .or. i_arg_ACTA /=0 .or.       &
    ...
    do i=1, nb_arg
     if (i/=i_arg_DEBUG .and. i/=i_arg_NOOUT .and. i/=i_arg_NOHKL .and. i/=i_arg_CIFDEP .and. i/=i_arg_ACTA  .and. &
      ...
      cmd_line = trim(new_cmd_line)// ' ' //trim(cmd_arg(i))
     endif
    end do
 ...
 ! >>>> interpretation de l'argument #1 de la ligne de commande
  ...
  if(len_cmd_line /=0) then
   ...
   !ELSEIF(input_arg(1:4) == 'P4P=') then
   ! keyword_P4P = .true.
   ! arg_keyword = .true.
   ! P4P_file_name = input_arg(5:)
   ...
   !ELSEIF(input_arg(1:4) == 'RAW=') then
   ! keyword_RAW = .true.
   ! arg_keyword = .true.
   ! RAW_file_name = input_arg(5:)
 ...
 if (keyword_CLA) then
  ...
  OPEN(UNIT=2, FILE='CRYSCALC_cla.txt', ACTION='write', STATUS='replace')
   ...
   call write_info('')
   call write_info('CRYSCAL commands line arguments are stored in the CRYSCALC_cla.txt file.')
   call write_info('')
   stop
 endif
