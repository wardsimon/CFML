...
subroutine  calcul_distances
  ...
  do i = 1, nb_dist_calc
   ...
   ok = .false.
   call get_label_atom_coord('dist', i,1, atom_index, ok)
   IF(.NOT. ok) cycle
   atom_1_coord(1:3) = new_coord(1:3)

   if(i1 == 0) then
    ok = .false.
    call get_label_atom_coord('dist', i,2, atom_index, ok)
    IF(.NOT. ok) cycle
    atom_2_coord(1:3) = new_coord(1:3)
    call distance_calculation(atom_1_coord(:), atom_2_coord(:), distance)
    WRITE(message_text,'(10x,3a,3(2x,F10.5))') '    . ', label_1(1:6), ' : ', atom_1_coord(1:3)
    call write_info(TRIM(message_text))
    WRITE(message_text,'(10x,3a,3(2x,F10.5))') '    . ', label_2(1:6), ' : ', atom_2_coord(1:3)
    call write_info(TRIM(message_text))
    WRITE(message_text,'(10x,5a,F10.5, a)')    '    . d(', label_1(1:6), ' - ', label_2(1:6), ') = ',distance, ' A'
    call write_info(TRIM(message_text))

   else         ! recherche de l'atome equivalent
    ok = .false.
    label_2 = label_2(1:i1-1)
    atom2_dist(i) = label_2
    call get_label_atom_coord('dist', i,2, atom_index,ok)
    call Generate_equiv_atom(label_1, label_2, atom_1_coord, atom_index, 'dist')
   end if
...
!!-----------------------------------------------------------------------------------------------------------------------
subroutine  calcul_differences
  use cryscalc_module, ONLY      : nb_diff_calc, new_coord, atom1_diff, atom2_diff, SP_value, nb_atom, message_text, &
                                   debug_proc
  USE IO_module
  ...
  if(debug_proc%level_2)  call write_debug_proc_level(2, "calcul_difference_vector")

  call write_info('')
  call write_info('    . ATOMIC VECTOR DIFFERENCE CALCULATIONS')
  call write_info('    ---------------------------------------')
  call write_info('')
  ...
  do i = 1, nb_diff_calc
   label_1 = atom1_diff(i)
   label_2 = atom2_diff(i)
   i1  = index(label_2, '_*')

   ok = .false.
   call get_label_atom_coord('diff', i,1, atom_index, ok)
   IF(.NOT. ok) cycle
   atom_1_coord(1:3) = new_coord(1:3)

   if(i1 == 0) then
    ok = .false.
    call get_label_atom_coord('diff', i,2, atom_index, ok)
    IF(.NOT. ok) cycle
    atom_2_coord(1:3) = new_coord(1:3)
    ...
    call write_info(TRIM(message_text))
    call write_info('')
    WRITE(message_text,'(10x,a,3(2x,F10.5))')  '    >> atomic coordinates difference vector: ', &
                                               atom_2_coord(1:3) - atom_1_coord(1:3)
    call write_info(TRIM(message_text))

   else         ! recherche de l'atome equivalent
    ok = .false.
    label_2 = label_2(1:i1-1)
    atom2_diff(i) = label_2
    call get_label_atom_coord('diff', i,2, atom_index,ok)
    call Generate_equiv_atom(label_1, label_2, atom_1_coord, atom_index, 'diff')
   end if
  ...
  return
end subroutine    calcul_differences
...
!-------------------------------------------------------------------------------------------------------------
 subroutine calcul_connectivity_atom
  use cryscalc_module,                ONLY : keyword_create_CIF, CONN_all, CONN_all_X, CONN_self, nb_atom, atom_CONN,  &
                                             CONN_species, CONN_out, CONN_excluded, CONN_ONLY_X, calcul_BVS,                     &
  ...
  implicit none
   integer                            :: i, i1, i2, i3, nd, na, n_a, n_out, nl, ns
   integer                            :: n_atm, n_atm1, n_ang, nb_dist_max
   integer                            :: l1, l2, len_string
   integer                            :: i_error
   character(len=256)                 :: read_line, adjusted_line, previous_line
   ...
   real                               :: r_eff, dist_AB, distorsion, dist_moy
   real, dimension(1000)              :: dist
  ...
  !
  if(.not. CONN_all .and. .not. CONN_ONLY_X) then
   atom_ok = .false.
   do i=1, nb_atom
    if(CONN_all_X) then
     l1 = len_trim(CONN_species)
...
!  idem Bond_str
   ...
   if(CONN_SELF) then
    call write_info('    !! Only distances between identical labels are output !!')
    call write_info('')
   end if

   if(CONN_ONLY_X) then
    call write_info('    !! Only distances between identical species are output !!')
    call write_info('')
   end if
     ...
     if(CONN_all) then
      do
       ...
       if(CONN_out_condensed) then
        ...
        if(.not. search_angles) then
         ...
         if(read_line(32:33) == '):') then
          ...
          if(CONN_ONLY_X) then
           ligand = read_line(26:31)
           ligand = adjustl(ligand)
           len_string = len_trim(CONN_species)
           if(ligand(1:len_string) /= ATOM_conn%excluded(1:len_string)) cycle
          end if

          call write_info('    '//read_line(16:45))    ! distance
         ...
         if(read_line(5:5) == '(' .and. read_line(14:14) == '(' .and. read_line(23:23) == '(') then
          n_ang = n_ang +1
          if(n_ang == 1) then
           call write_info(trim(read_line))
        ...
        if(len_trim(read_line) /=0) then
         if(index(read_line, 'Orig. extr. p.equiv.')/=0) then
          call write_info('')
          call write_info(trim(read_line))
          call write_info('')
         elseif(index(read_line, '---------------') /=0 .and. len_trim(previous_line)==0) then
          call write_info('')
          call write_info(trim(read_line))
         elseif(index(read_line, 'Bond type:') /=0) then
          call write_info('')
          call write_info(trim(read_line))
         else
          call write_info(trim(read_line))
      ...
      do
       ...
       !call write_info('...')
        ...
        if(CONN_ONLY_X) then
         len_string = len_trim(CONN_species)
         if(current_atom(1:len_string) /= CONN_species(1:len_string)) cycle
        end if

        if(u_case(ATOM_CONN%label) == u_case(current_atom) .or. (CONN_all_X .and. CONN_out(n_out)) .or. CONN_ONLY_X) then
         call write_info(trim(read_line))
         read(55, '(a)', iostat=i_error) read_line
         if(i_error /=0) exit
         call write_info(trim(read_line))
         ...
         do
          ...
          if(index(read_line(16:16), '(') /=0) then
           i1 = index(read_line(32:44), '(')  ! avec esd ?
           if (i1 ==0) then
            i2 = 45
            i3 = 45
           else
            i2 = 30 + i1
            i1 = index(read_line(33:45), ')')  ! avec esd ?
            if(i1 /=0) then
             i3 = 33 + i1
            else
             i3 = 45
           ...
           !read(read_line(32:39),*, iostat=i_error) dist_AB
           !read(read_line(32:i2),*, iostat=i_error) dist_AB
           !read(read_line(34:i2),*, iostat=i_error) dist_AB
           i1 = index(read_line, ':')
           if(i1 /=0) then
            read(read_line(i1+1:i2),*, iostat=i_error) dist_AB
           end if
          ...
          ! compare avec atome a exclure
           ...
           ! compare avec atomes meme espece
           if(CONN_ONLY_X) then
            !ligand = read_line(25:29)
            ligand = read_line(26:31)
            ligand = adjustl(ligand)
            len_string = len_trim(CONN_species)
            !write(*,*) ' X ligand : ', CONN_species(1:len_string),  '   ' , ligand(1:len_string)
            !pause
            if(ligand(1:len_string) /= CONN_species(1:len_string)) cycle
           end if


           nl = nl + 1
           if(nl > nb_dist_max) then
            call write_info('')
            call write_info(' !! Number of calculated distances too large (max=1000).')
            call write_info('    Try remove H atoms in the distances calculation ("no_H" argument) ?')
            call write_info('')
            return
           end if
           dist(nl) = dist_AB
           ...
           if(CONN_out_condensed) then
            if(CONN_SELF .and. read_line(18:22) /= read_line(27:31)) cycle
             if(i1==0) then
              write(message_text, '(5x,i3,2a)') nl, '. ', trim(read_line(16:i3))//')'
              call write_info(trim(message_text))
             !call write_info('   '//trim(read_line(16:i3))//')')
            else
             write(message_text, '(5x,i3,2a)') nl, '. ', read_line(16:i3)
             call write_info(trim(message_text))
             !call write_info('   '//read_line(16:i3))
            end if
            if(keyword_create_CIF)  call write_dist_CIF(read_line)
            cycle
           end if
          ...
          if(CONN_SELF) then
           ...
           if(read_line(18:22) == read_line(27:31)) then
            call write_info(trim(read_line))
            ns = ns + 1
           end if
          else
       ...
       ! calcul de la distorsion du polyedre ------------------------------------
         if(nl > 1 .and. ns ==0 .and. .not. CONN_SELF .and. .not. CONN_ONLY_X) then
          distorsion  = 0.
          dist_moy    = 0.
          dist_moy = sum(dist(1:nl)) / nl
          do i = 1, nl
           distorsion = distorsion + ((dist(i) - dist_moy) / dist_moy)**2.
          end do
          distorsion = distorsion / nl
          call write_info('')
          write(message_text, '(3x,a,F8.4)') ' Mean distance        = ', dist_moy
          call write_info(trim(message_text))
          write(message_text, '(3x,a,I3)')   ' Number of ligands    = ', nl
          call write_info(trim(message_text))
          write(message_text, '(3x,a,F8.4)') ' Polyedron distorsion = ', distorsion
          call write_info(trim(message_text))
          call write_info('')
         end if
 ! ------------------------------------------------------------------------
          ...
          if(CONN_out_condensed) then
           ...
           if(adjusted_line(1:1) == '(') then
            ...
            if (na ==1) then
             !call write_info(trim(read_line))
             n_a = n_a + 1
             write(message_text, '(5x,i3,2a)') n_a, '. ', trim(read_line)
             call write_info(trim(message_text))
...
subroutine calcul_connect()
 ...
 USE CFML_Math_3D,                   only : Polyhedron_Volume
  ...
  ok = .false.
  call get_label_atom_coord('dist', 1,1, atom_index, ok)
  if(.not. ok) return
  atom_1_coord(1:3) = new_coord(1:3)
...
! extract from Bond_Str (JRC, JGP)
   ...
   USE cryscalc_module,                ONLY  : debug_proc
    ...
    implicit none
      ...
      allocate(bond_spec(n_spec,n_spec), stat=ier)
      if(ier/=0) call write_alloc_error("bond_spec")
      bond_spec=0
...
!--------------------------------------------------------
 subroutine Generate_equiv_atom(label_1, label_2, atom_1_coord, atom_index, input_string)
  USE Cryscalc_module, only : nb_symm_op, CONN_dmin, CONN_dmax, atom_coord, new_coord, message_text, symm_op_string, &
                              R => symm_op_rot,  T => symm_op_trans, atom_coord
 USE IO_module
  implicit none
   CHARACTER(LEN=16),  intent(in)      :: label_1, label_2
   REAL, DIMENSION(3), intent(in)      :: atom_1_coord
   CHARACTER(LEN=*),   intent(in)      :: input_string
   integer                             :: atom_index
   real, dimension(3)                  :: tr_coord
   integer                             :: i_op, n
   integer                             :: tx, ty, tz
   integer                             :: tx_1, tx_2
   integer                             :: ty_1, ty_2
  ...
  if(input_string(1:4) == 'dist') then
   ...
   tz_1 = -2
   tz_2 =  2
  elseif(input_string(1:4) == 'diff') then
   tx_1 = -1
   tx_2 = 1
   ty_1 = -1
   ty_2 = 1
  ...
  n = 0
   do i_op = 1, nb_symm_op
    new_coord(1) = R(1,1, i_op) * atom_coord(1, atom_index)  +  R(1,2, i_op) * atom_coord(2, atom_index) +  &
                   R(1,3, i_op) * atom_coord(3, atom_index)  + T(1, i_op)
    new_coord(2) = R(2,1, i_op) * atom_coord(1, atom_index)  +  R(2,2, i_op) * atom_coord(2, atom_index) +  &
                   R(2,3, i_op) * atom_coord(3, atom_index)  + T(2, i_op)
    new_coord(3) = R(3,1, i_op) * atom_coord(1, atom_index)  +  R(3,2, i_op) * atom_coord(2, atom_index) +  &
                  R(3,3, i_op) * atom_coord(3, atom_index)  + T(3, i_op)

    !call distance_calculation(atom_1_coord(:), new_coord(:), distance)

    do tx= tx_1, tx_2
     do ty= ty_1, ty_2
      do tz= tz_1, tz_2
       tr_coord(1) = new_coord(1) + tx
       tr_coord(2) = new_coord(2) + ty
       tr_coord(3) = new_coord(3) + tz
       if(input_string(1:4) == 'dist') then
        call distance_calculation(atom_1_coord(:), tr_coord(:), distance)
        if(distance > CONN_dmin .and. distance < CONN_dmax) then
         WRITE(message_text,'(10x,3a,3(2x,F10.5))') '    . ', label_1(1:6), ' : ', atom_1_coord(1:3)
         call write_info(TRIM(message_text))
         write(symm_string, '(5a)')  trim(symm_op_string(1, i_op)), ',', trim(symm_op_string(2, i_op)), ',',  &
                                     trim(symm_op_string(3, i_op))
         WRITE(message_text,'(10x,3a,3(2x,F10.5),5x,a,3I2,a,3x,a)') '    . ', label_2(1:6), ' : ', tr_coord(1:3), &
                                                              '(', tx,ty,tz, ')', trim(symm_string)
      ...
      elseif(input_string(1:4) == 'diff') then
       n = n + 1
       call write_info('')
       WRITE(message_text,'(10x,a,I6,a)') '    #### ', n, '  ###'
       call write_info(TRIM(message_text))
       WRITE(message_text,'(10x,3a,3(2x,F10.5))') '    . ', label_1(1:6), ' : ', atom_1_coord(1:3)
       call write_info(TRIM(message_text))
       write(symm_string, '(5a)')  trim(symm_op_string(1, i_op)), ',', trim(symm_op_string(2, i_op)), ',',  &
       ...
       WRITE(message_text,'(10x,3a,3(2x,F10.5),5x,a,3I2,a,3x,a)') '    . ', label_2(1:6), ' : ', tr_coord(1:3), &
                                                              '(', tx,ty,tz, ')', trim(symm_string)
       call write_info(TRIM(message_text))
       WRITE(message_text,'(10x,a,3(2x,F10.5))')  '    >> atomic coordinates difference vector: ', tr_coord(:) - atom_1_coord(1:3)
       call write_info(TRIM(message_text))
     end if
    end do
   end do
 ...
 return
end   subroutine Generate_equiv_atom
