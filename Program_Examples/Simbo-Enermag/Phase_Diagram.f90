   ...
   Program Phase_diagram
      ...
      0.33333,0.50000,0.33333,  0.33333,0.33333,0.50000,  0.50000,0.33333,0.33333 /),(/3,84/) )
   ...
   !---- Symmetry parameters ----!
      ...
      write(*,'(a/)') '   ------------------------------------------------------------------------ '
      write(*,'(a)') ' => Code of the *.res file: '
      read(*,'(a)') fileres
      open(unit=1,file=trim(fileres)//'.res',status='old')
      open(unit=3,file=trim(fileres)//'.ana',status='unknown')

      write(*,'(a)') ' => Number of Js (only the first 3 Js may be varied!): '
      read(*,*) nj
      write(*,'(a)') ' => Number of Sites: '
      read(*,*) ns
      nsub=0
      do i=1,ns
       write(*,'(a,i2,a)') ' => Number of atoms for site:',i,': '
       read(*,*) nats(i)
       nsub=nsub+nats(i)
      end do
  ...
  !      write(2,form) valj(1:nj), j, magt(1:nsub), vk(:)
       ...
       write(*,'(a)') '    Density = User defined   (u) (<cr>=u)?: '
       read(*,'(a)') ans
       ...
       if( ans == 'o' .or. ans == 'O') then
         ...
         do i=1,num_mag
             write(*,'(a,i3,2x,a,i8,a)') ' => Density value for',i,trim(magchar(i)), freq_mag(i),': '
             read(*,*) freq_mag(i)
         end do
         forall (i1=1:ngrid(1), i2=1:ngrid(2), i3=1:ngrid(3))
  ...
  !!! Complementary variables for Fourier compatibility
      ...
      !---- Opening binary file  ----!
      open(jfilbin,file=trim(fileres)//'.bin',form='unformatted',status='unknown')
