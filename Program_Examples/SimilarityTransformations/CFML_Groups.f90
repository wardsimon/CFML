   Module CFML_Groups
    ...
    logical            :: Err_group
    character(len=256) :: Err_group_mess
    ...
    Subroutine Group_Constructor(generatorList,mnum_op,Grp)
       character(len=*),   intent(in)     :: generatorList
       integer,            intent(in)     :: mnum_op
       Class (Group_Type), intent(in out) :: Grp
       !--- Local variables ---!
       character(len=40),dimension(:),allocatable :: gen
       type(Symm_Oper_Type), dimension(mnum_op)   :: Op
       ...
       type(rational), dimension(:,:),allocatable :: Mat
       !
       allocate(gen(mnum_op))
       call Get_Operators_From_List(generatorList,d,ngen,gen)
       allocate(Mat(d,d))
       do i=1,mnum_op
         call Allocate_Operator(d,Op(i))
       end do
    ...
    End Subroutine Group_Constructor

    Subroutine Get_Operators_From_List(generatorList,d,ngen,gen)
       character(len=*),              intent(in)  :: generatorList
    ...
    End Subroutine Get_Operators_From_List
   ...
   Program test_groups
     ...
     do
         ...
         call Group_Constructor(generatorList,maxnum_op,Grp)
         if (Err_group) then
