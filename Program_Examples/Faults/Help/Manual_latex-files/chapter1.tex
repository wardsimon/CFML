...
The resulting code is the FAULTS program \cite{FAULTSPaper1,FAULTSPaper2}. The program  is distributed in the hope that it will be useful, but without any warranty of being free of internal errors.
...
If the user uses FAULTS as a separate program, it will be launched from a DOS window, after having copied the file data.scf into the working folder.
...
\caption{\bf Example of observed and calculated profile plot}
\label{pgf}
\end{center}
\end{sidewaysfigure}
...
\label{running}
 ...
 \begin{figure}[hbtp]
\begin{center}
\includegraphics [width= 3.5 in]{faultslayout.jpg}
...
During the refinement process, depending on the option chosen by the user (see the comments on the keyword \textit{Nprint} in the \textit{Calculation} subsection page \pageref{nprint}), the DOS window will display the values of the agreement factors at the end of each evaluation, and optionally the list of the previous and new refinable parameters at each evaluation step.
 ...
 \begin{figure}[hbtp]
\begin{center}
\includegraphics [width= 5.0 in]{faultslayout2.jpg}
