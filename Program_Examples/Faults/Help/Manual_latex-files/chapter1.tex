...
The resulting code is the FAULTS program \cite{FAULTSPaper1,FAULTSPaper2}.   Note that FAULTS, as DIFFaX, can also simulate electron diffraction patterns.  \\

The program  is distributed in the hope that it will be useful, but without any warranty of being free of internal errors.
The authors acknowledge all suggestions and notification of possible bugs found in the program.\\


\section{Download and installation}

FAULTS can be obtained either as part of the FullProf Suite \cite{FPDownload, FPPaper, FPManual} at \url{www.ill.eu/sites/fullprof/}, or as a separate program by downloading the compressed file FAULTS.zip at \url{www.cicenergigune.com/}.\\

In the case the user chooses to install FAULTS with the FullProf Suite, the program FAULTS will be launched directly from the FullProf Suite Toolbar.
If the user uses FAULTS as a separate program, it will be launched from a DOS window, after having copied the standard scattering factor data file \textit{data.sfc} into the working folder.
...
unit cells of a determined symmetry. Nevertheless, to use FAULTS, as in the case of DIFFaX \cite{DiffaxManual}, one needs to think of crystals in terms
...
The FAULTS input file is similar to the one used in DIFFaX \cite{DiffaxManual}, although there are some differences that should be taken into account to avoid errors. A program that permits to convert DIFFaX input files into FAULTS one is available at \url{www.cicenergigune.com/}.\\  
...
\item \textbf{Optional commands for visualization of the structure with FullProf Studio:} To simplify the construction of the structural section and to avoid errors, it is recommended to regularly draw the structure with the help of the visualization program FullProf Studio \cite{FPSDownload,FPSManual}. To do so, the user should ask the program to generate an output file \textit{.fst} suitable for FullProf Studio (see section \ref{fstf}), by entering an optional line beginning by the keyword \textit{FST\_CMD}, followed by the keyword \textit{SEQ}, the number of layers and the sequence of the layers to be drawn; for example:
...
Then the user can add as many optional lines as needed, each of them beginning with the keyword \textit{FST\_CMD}, followed by the required FullProf Studio commands (see the FullProf Studio manual for
more details \cite{FPSManual}). For example, to control the drawing of Ni-O bonds (of length between 0.0 and 1.2 \AA) polyhedra of the example input file, the limit of the box and the position of the first layer in the artificial FullProf Studio cell, the user may add the following lines:
...
\begin{itemize}
\item \textbf{Simulation:} In order to simulate a powder diffraction pattern the user has to specify the keyword \textit{Powder} followed by the values of  2$\theta_{min}$,  2$\theta_{min}$ and the step  (in degrees).
For example,
...
\hspace*{1.5 cm}Powder 5.0 90.0 0.02 \= \{\ 2$\theta_{min}$, 2$\theta_{max}$, step \}\
\end{tabbing}
\end{flushleft}
%\footnotesize{
...
Finally, whatever the kind of calculation to be performed (SIMULATION or LMQ), the user can add an optional line with the instruction \textit{Replace\_Files} at the end of the \emph{Calculation} section to make the program replace existent output files by the ones generated at the end of a new run, instead of creating new files.
...
If the user chooses to treat the background as a linear interpolation between N given points, he/she should provide a file which contains the list of points that will be used to calculate the background at each value of the scattering variable. The user can create such a file from the experimental data using the program WinPLOTR \cite{WinplotrPaper} of the FullProf Suite \cite{FPDownload, FPManual}.
