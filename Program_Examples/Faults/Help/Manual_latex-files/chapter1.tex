...
The other possibility to start creating an input .flts file is to create a template from CIF file. To do this one can call faults program from command line and provide three arguments. The first argument is a target CIF file including extension (i.e. "file.cif"). The second argument is number of layers within the structure. The third one is a thickness of the layers expressed as cartesian coordinate (for example 0.333).\\
There is some restrictions using that function:\\
1) structure described in CIF file should have P1 symmetry \\
2) atoms lying on the front face of the layer are relate to the next layer.
