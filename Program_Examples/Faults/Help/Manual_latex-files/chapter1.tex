...
more details \cite{FPSManual}). For example, the keyword \textit{CONN} can be used to control the drawing of bonds (\textit{e.g.} Ni-O bonds of length between 0.0 and 2.0 \AA in the example below), the keyword \textit{BOX} allows to control the limits of the box, and the keyword \textit{stack\_vect} permits to set the position of the first layer in the artificial FullProf Studio cell:

\begin{flushleft}
\begin{tabbing}
	\hspace*{1.50 cm}FST\_CMD  \=CONN \= \=NI O \=0.0 2.0\\*
