...
The other possibility to start creating an input .flts file is to create a template from CIF file. To do this one can call faults program from command line and provide three arguments. The first argument is a target CIF file including extension (i.e. "file.cif"). The second argument is the number of layers within the structure. The third one is a thickness of the layers expressed as cartesian coordinate (for example 0.333).\\
...
\end{tabbing}	
\end{flushleft}
and to fix the position of the first layer in the big artificial box, the user may add the following line:
\begin{flushleft}
\begin{tabbing}
