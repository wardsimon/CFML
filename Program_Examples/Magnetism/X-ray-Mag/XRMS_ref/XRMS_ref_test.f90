...
!
   ...
   allocate(hkl_angles(sample_refl_list%nref,6))
   call read_hkl_angles(sample_refl_list,ub_matrix,hkl_angles)
   ...
   call Levenberg_Marquardt_Fit(intensities_nder,dat%nobs,cond,vs,chi2,inform)
   ...
   do i=1,sample_refl_list%nref
      m_refl = sample_refl_list%mh(i)
      write(unit=*,fmt='(3f9.4,2f12.5)') m_refl%h, m_refl%fobs2, m_refl%sfobs2
   end do
