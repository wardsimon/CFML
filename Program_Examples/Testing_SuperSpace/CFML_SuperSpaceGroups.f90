  ...
  Contains
    ...
    subroutine identity_rational(d, identity)
      ...
      integer :: j, k
      forall ( j = 1:d, k = 1:d, j /= k ) identity(j,k) = 0//1
      forall ( j = 1:d, k = 1:d, j == k ) identity(j,k) = 1//1
    ...
    Subroutine Sylvester_Solver_sp(A,B,C,X,nsol,ok)
    	...
    	real(kind=sp), parameter :: eps=0.000001
    	...
    	homogeneous=.false.
    	if(sum(abs(C)) < eps) homogeneous=.true.
    	U=Kronecker_Product(Idm,A)-Kronecker_Product(transpose(B),Idn)
    	vC=vectorize(C)
      X=0.0_sp
      call SVDcmp(U,w,V)
      singular=.false.
      if(any(abs(w) < eps)) singular=.true.
