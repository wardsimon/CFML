 ...
 ! >>>>  analyse de la ligne de commande
  ...
  if(nb_arg /=0) then
   ...
   if(LOG_file%write) then
    open(unit = LOG_file%unit, file = 'cryscal.log')
 ...
 ! >>>> interpretation de l'argument #1 de la ligne de commande
  if(len_cmd_line /=0) then  
   ...
   if(.not. keyword_HELP .and. .not. keyword_KEY .and. .not. keyword_create_CRYSCAL_HTML &
    ...
    open(UNIT=2, FILE='cryscal.out',        ACTION='write', status='replace')   
   endif 
...
subroutine end_of_program
 ...
 call WRITE_info(' ')
 call WRITE_info(' ')
 call WRITE_info('   All CRYSCAL results are stored in CRYSCAL.out file. ')
 call WRITE_info(' ')
