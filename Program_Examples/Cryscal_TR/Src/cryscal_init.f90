...
!-------------------------------------------------------------------------------
 ...
 !OBV_REV_twin_matrix  = 2            ! matrice de maclage:  ( 0-1 0 -1 0 0  0 0-1): twofold axis parallel to (a-b)'
  ...
  CIF_parameter_SUPERNOVA%diffrn_radiation_monochromator   = 'mirror'
 ...
 subroutine read_cryscal_ini()
  USE cryscal_module, ONLY : debug_proc, INI_unit, cryscal_ini, winplotr_exe, my_editor, my_browser, my_word, &
                             my_pdflatex, WEB, AUTHOR, DEVICE,                                                &
                             cryscal_css, cryscal_report_css,                                                 &
                             ...
                             message_text, cryscal_path_name, winplotr_path_name, on_screen
  ...
  implicit none
   ...
   ! recherche du nom du repertoire associé à CRYSCAL
   ...
   if(long /=0) then
    ...
    cryscal_css = trim(cryscal_path_name)//'\cryscal.css'
    cryscal_report_css = trim(cryscal_path_name) // '\cryscal_report.css'
   endif


  ! recherche du fichier cryscal.ini
  inquire(file='cryscal.ini', exist = file_exist)
  if(file_exist) then
   cryscal_ini = 'cryscal.ini'
  else
   ...
   if(long /=0) then
    ...
    if(len_trim(cryscal_ini) == 0) then
      cryscal_ini  = trim(winplotr_path_name)//'\cryscal.ini'
     !cryscal_css = trim(cryscal_path_name) // '\cryscal.css'
     !cmryscal_report_css = trim(cryscal_path_name) // '\cryscal_report.css'
    endif
   endif
   ...
   if(len_trim(cryscal_ini) == 0 .and. len_trim(winplotr_exe) == 0) then
    if(debug_proc%write) then
     ...
     call write_debug_proc('No environment variable defined for CRYSCAL.', '')
     call write_debug_proc('', '')
    endif
    return
   endif
   ...
   if(debug_proc%write) then
    call write_debug_proc('','')
    call write_debug_proc('CRYSCAL_ini        :', trim(cryscal_ini))
    !call write_debug_proc('','')
    call write_debug_proc('CRYSCAL_css        :', trim(cryscal_css))
    !call write_debug_proc('','')
    call write_debug_proc('CRYSCAL_report_css :', trim(cryscal_report_css))
    call write_debug_proc('','')
   endif
   ...
   IF(.NOT. file_exist) then
    if(debug_proc%write) then
     call write_debug_proc('','')
     call write_debug_proc('!! CRYSCAL.ini does not exist.','')
     call write_debug_proc('','')
...
!-------- CIF COMMAND LINE ARGUMENTS --------------------------------------------------------------------
   ...
   if(debug_proc%write) then
    ...
    if (keyword_create_CFL) then
     call write_debug_proc ('create_CFL',      '1     ! .CFL file for CRYSCAL')
    else
     call write_debug_proc ('create_CFL',      '0     ! .CFL file for CRYSCAL')
    endif
...
subroutine output_setting
 USE cryscal_module, ONLY :  INI_unit, cryscal_ini, my_browser, my_editor, my_pdflatex, WEB, DEVICE, AUTHOR, message_text,     &
 ...
 call write_info(trim(message_text))
 if(include_RES_file) then
  call write_info('   > include_RES_file           = 1           ! include .RES file in the archive_cryscal.cif file')
 else
  call write_info('   > include_RES_file           = 0           ! include .RES file in the archive_cryscal.cif file')
 endif
 if(update_parameters) then
...
subroutine open_debug_file
 ...
 implicit none
  ...
  open(unit = debug_proc%unit, file = 'cryscal_debug.txt', status = "replace", iostat=i_error)
	if(i_error /=0) then
     call write_info('')
     call write_info('  !! Problem opening cryscal_debug.txt file !!')
     call write_info('')	
	 debug_proc%write = .false.
	end if
