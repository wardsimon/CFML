 ...
 Module Gen_Powder_Pattern
    ...
    Type, public :: PowPat_Laue
       integer :: n_lambda
       real    :: Lambda_min, Lambda_max
       real    :: Lambda_cent,Lambda_width
       real    :: temp
       integer :: n_holes
       real,dimension(5) :: pholes,percent,pwidth
    End Type PowPat_Laue
 ...
 Contains
    ...
    Subroutine Calc_Powder_Pattern(Ppc,PL,Hkl,Pat)
       !---- Argument ----!
       Type(PowPat_CW_Conditions),     intent(in)  :: PPC
       Type(PowPat_Laue),              intent(in)  :: PL
       Type(Reflection_List_Type),     intent(in)  :: hkl
       Type(Diffraction_Pattern_Type), intent(out) :: Pat

       !--- Local Variables ----!
       integer :: i,j,npts,i1,i2,k,i_spec=15
       real    :: step,Intens,Bragg,Hl,Hg, ss,cs,tt,th1,th2,LorentzF, Y,eta,fwhm,chw,lambda,flux
       ...
       step= (PL%lambda_max-PL%lambda_min)/real(PL%n_lambda-1)
       write(*,*) PL%n_lambda,  PL%lambda_min,  PL%lambda_max
       open(unit=i_spec,file="spectrum.xys",status="replace",action="write")
       write(unit=i_spec,fmt="(a)")"TITLE: Spectrum for calculating the Laue Powder Pattern"
       write(unit=i_spec,fmt="(a)")"Scattering variable: Lambda"
       write(unit=i_spec,fmt="(a)")"Legend_X   Lambda (Angstroms)"
       write(unit=i_spec,fmt="(a)")"Legend_Y   Flux (arb. units)"
       ...
       do k=1,PL%n_lambda
         ...
         do i=1,hkl%nref
            ...
            i1=max(i1,1)
            i2=min(i2,npts)
            Intens= LorentzF *hkl%ref(i)%mult * hkl%ref(i)%Fc**2 * flux  ! PPC%Scalef
            do j=i1,i2
    ...
    Function Get_Gauss(x,xc,H) result(fl)
     real, intent(in) :: x,xc,H
     real             :: fl
     real :: v, bg
       v= x-xc
    ...
    Function Maxwellian(Lambda,PL) result(Intm)
      ...
      real (kind=dp):: kbt,hom,ex,l2,intholes
      ...
      intholes=0.0
      do i=1,PL%n_holes
        intholes=intholes+PL%percent(i)*get_Gauss(Lambda,PL%pholes(i),PL%pwidth(i))
      end do
      intholes=max(0.0,intholes)
      Intm=Intf*exp(-ex)/(l2*l2)*(1.0 - intholes)
      return
  ...
  Program Laue_Powder
     ...
     write(unit=*,fmt="(/,/,6(a,/))")                                                     &
          "            ------ PROGRAM LAUE POWDER PATTERN CALCULATION  ------"          , &
          "                  ---- Version 0.1 February-2014----"                        , &
          "    **********************************************************************"  , &
          "    *     Laue powder diffraction pattern from a *.CFL or a *.CIF file   *"  , &
          "    **********************************************************************"  , &
          "                          (JRC- February-2014 )"
     write(unit=*,fmt=*) " "
       ...
       if (mode == "CFL") then
          do i=1,fich_cfl%nlines
             ...
             Select Case(line(1:k-1))
                ...
                Case("LAMBDA_RANGE")
                   read(unit=line(14:),fmt=*,iostat=ier) PPLaue%Lambda_min, PPLaue%Lambda_max, PPLaue%n_Lambda
                   if(ier /= 0) then
                     PPLaue%Lambda_min=0.5; PPLaue%Lambda_max=5.0; PPLaue%n_Lambda=1200
                   end if

                Case("GAUSSIAN_RANGE")
                   read(unit=line(15:),fmt=*,iostat=ier) PPLaue%Lambda_cent, PPLaue%Lambda_width
                   if(ier /= 0) then
                     PPLaue%Lambda_cent=1.0; PPLaue%Lambda_width=5.0
                   end if

                Case("MAXWELL_TEMP")
                   read(unit=line(13:),fmt=*,iostat=ier) PPLaue%temp
                   if(ier /= 0) then
                     PPLaue%temp=300.0
                   end if

                Case("HOLES")
                   read(unit=line(6:),fmt=*,iostat=ier) PPLaue%n_holes, &
                         (PPLaue%pholes(j),PPLaue%percent(j),PPLaue%pwidth(j),j=1,PPLaue%n_holes)
                   if(ier /= 0) then
                     PPLaue%n_holes=0
                   end if
                   write(*,*)   PPLaue%n_holes,"  holes read!"
                   do j=1, PPLaue%n_holes
                      write(*,*)   "  Position, % and width ",PPLaue%pholes(j),PPLaue%percent(j),PPLaue%pwidth(j)
                   end do
    ...
    Mult=2*SpG%NumOps
      ...
      call Calc_powder_pattern(PPC,PPLaue,hkl,Pat)
      call cpu_time(tfin)
      tim=tim+ tfin-tini
