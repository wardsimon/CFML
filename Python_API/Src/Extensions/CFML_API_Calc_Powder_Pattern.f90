...
!!$subroutine compute_powder_pattern(input_filename, mode, job_info, Pat)
!!$     !---- Use Modules ----!
...
!!$     else
!!$        fich_cfl%nlines=0
!!$        call Readn_set_Xtal_Structure(trim(input_filename),Cell,SpG,A,Mode="CFL",file_list=fich_cfl. Job_info=job_info)
!!$        if (Mode .eq. "CFL") then
!!$           !
