...
subroutine init_powder_generation() bind(c, name="init_powder_generation")
  type(c_ptr) :: m
  m = init()
end subroutine
...
function crysfml_powder_compute_powder_pattern(self_ptr, args_ptr) result(r) bind(c)
 ...
 ! Check if the arguments are OK
  ...
  if (num_args /= 1) then
    call raise_exception(TypeError, "function expects exactly 1 argument")
    call args%destroy
    return
  endif
  ierror = args%getitem(filename_object, 0)
  ierror = cast_nonstrict(filename, filename_object)

  call compute_powder_pattern(filename, Pat)
...
subroutine compute_powder_pattern(filcod,Pat)
     !---- Use Modules ----!
     !use f2kcli
 ...
 !!
 !!
        !filcod(1:len_trim(filename)) = filename(1:len_trim(filename))
        !filcod = filename
        !!write(*,*) filename
...
!program compute_powder

!use powder_generation
!call compute_powder_pattern("Si_Laue")!,powder_pattern)
