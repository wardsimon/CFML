   ...
   !!----      -2 : FORTRAN read error
    ...
    Subroutine Get_MatVec_from_String(string,mat,v,cod)
       ...
       if (i /= 0) then
          ...
          call Get_Mat_From_String(cMat,cd(1:3),mat)
          if (ERR_String) then
          ...
          if (nc /= 2)then
             ...
             call read_frac_string(ori(1:pos(1)-1),v(1))
             call read_frac_string(ori(pos(1)+1:pos(2)-1),v(2))
             call read_frac_string(ori(pos(2)+1:),v(3))
             if (ERR_String) then
    ...
    Subroutine Get_Vec_From_String(String,Cod,V)
       ...
       do m=1,nsp
          ...
          call read_frac_string(split(m), v(k))
    ...
    Subroutine Get_Separator_Pos(String,Sep,pos,ncar)
       ...
       do i=1,len_trim(String)
          j=j+1
          if (String(j:j) == '"') then  !A chains of characters is found, advance up the the next "
             do k=1,len_trim(String)    !the character "car" is ignored if it is within " "
                j=j+1
                if (String(j:j) /= '"') cycle
                exit
             end do
    ...
    !!----    If 'end_string' is present, return the number of lines until 'end_string' is founded
    ...
    Subroutine Number_Lines(filename,n, end_string)
       ...
       do
          ...
          if (present(end_string)) then                                         ! TR may 2013
             if(u_case(read_line(1:long)) == u_case(end_string(1:long))) exit
          end if
          n=n+1
       end do
    ...
    Subroutine Read_Fract_String(str,valu)
       ...
       return
    End Subroutine Read_Fract_String
    ...
    Subroutine SetNum_Std(Value, Std, string)
       ...
       string=trim(string)//"("
       n=len_trim(string)
       np=len(string)-n-1             !number of available places for writing
       aux=" "
       ...
       if(long > np) then
         string=string(1:n)//"*)"
          ...
          string=string(1:n)//trim(aux)//")"
       end if
       string=pack_string(string)
