   ...
   Subroutine Write_Eoscal_File(Pmin,Pmax,Pstep,Tmin,Tmax,Tstep,Tscale_In,Eos,Lun,Nprint)
      ...
      outer: do
         ...
         inner: do
            ...
            !> pressure (no esd)
            text=trim(format_R(parout(1),ip(1)))

            !> T value (no esd)
            text=trim(text)//'  '//trim(format_R((parout(2)),ip(2)))

            !> other params
            do i=3,11
                text=trim(text)//'  '//trim(format_R(parout(i),ip(i)))//' '//trim(format_R(esdout(i),ip(i)))
            end do
            if (eos%itran > 0)text=trim(text)//'  '//trim(format_R(parout(12),ip(12)))
