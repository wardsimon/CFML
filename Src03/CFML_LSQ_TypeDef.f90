...
Module CFML_LSQ_TypeDef
   ...
   !!----     integer          :: nfev            ! number of function evaluations (output component,
   !!----                                         ! useful for assessing LM algorithm)
   ...
   Type, public :: LSQ_Conditions_Type
      ...
      integer          :: nfev=0          ! number of function evaluations
   ...
   !!----     real(kind=cp),    dimension(:),allocatable :: x     !Vector containing a relevant quantity for each observation
   !!----                                                         !(x-coordinate ...)
   ...
   !!----     logical                                    :: code_comp  !If .true. the codes are interpreted as number in the LSQ
