    ...
    Subroutine Read_Pattern_Free(i_dat,Pat)
       ...
       do
          ...
          if (aline(1:1) == "!" .or. aline(1:1) == "#") then
            ...
            if(i /= 0) then
              pat%legendX=adjustl(aline(i+8:))
            end if
            i=index(aline,"Legend_Y")
            if(i /= 0) then
              pat%legendY=adjustl(aline(i+8:))
            end if
       ...
       if (present(mode)) then
          select case (u_case(mode))
              ...
              case ("ISIS")
                 ...
                 dif_pat%legendX =  "TOF (micro-seconds)"
                 dif_pat%legendY =  "Intensity (arb. units)"
    ...
    Subroutine Read_Pattern_One(Filename,Dif_Pat, Mode)
       ...
       select case (modem)
          case ("D1B" , "D20")
             ...
             dif_pat%legendX =  "2theta(degrees)"
             dif_pat%legendY =  "Intensity (arb. units)"
          ...
          case default
             ...
             dif_pat%ct_step = .true.
             if(len_trim(dif_pat%legendY) == 0) dif_pat%legendY =  "Intensity (arb. units)"
             if(len_trim(dif_pat%legendX) == 0)  then
                if(dif_pat%x(dif_pat%npts) > 180.0 ) then
                    dif_pat%scat_var =  "TOF"
                    dif_pat%legendX =  "TOF(micro-seconds)"
                else
    ...
    Subroutine Read_Pattern_XYSigma(i_dat,Pat)
       ...
       IF(txt1(1:6) /= "XYDATA") THEN
           ...
           do
              ...
              if(i /= 0) then
                 pat%legendX = adjustl(txt1(i+8:))
                 cycle
              end if
              i=index(txt1,"Legend_Y")
              if(i /= 0) then
                 pat%legendY=adjustl(txt1(i+8:))
                 cycle
              end if
           ...
           do i=1,5
              ...
              if(j /= 0) then
                 pat%legendX=adjustl(txt1(j+8:))
              end if

              j=index(txt1,"Legend_Y")
              if(j /= 0) then
                 pat%legendY=adjustl(txt1(j+8:))
              end if
