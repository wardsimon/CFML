...
rem
   g95 -c CFML_GlobalDeps_Windows.f90         -O0 -ftrace=full  -std=f2003    >  out
rem
   g95 -c CFML_math_gen.f90         -O0 -ftrace=full  -std=f2003    >> out
   g95 -c CFML_LSQ_TypeDef.f90      -O0 -ftrace=full  -std=f2003    >> out
...
rem
   ...
   g95 -c CFML_ILL_Instrm_data.f90  -O0 -ftrace=full                >> out
rem
   echo **---- Level 3 ----**
   echo .... Reflections, Atoms
rem
   g95 -c CFML_Eos_Mod.f90          -O0 -ftrace=full  -std=f2003    >> out
...
rem
   echo **---- G95 Directory ----**
rem
   if not exist ..\G95_debug mkdir ..\G95_debug
   if exist ..\G95_debug\LibC rmdir ..\G95_debug\LibC /S /Q
   mkdir ..\G95_debug\LibC
rem
   copy *.mod ..\G95_debug\LibC > nul
   move *.a   ..\G95_debug\LibC > nul
