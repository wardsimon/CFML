rem
rem CrysFML for Lahey Compiler (Debug) + WINTERACTER
rem
   @echo off
   cd %CRYSFML%\Src
rem
...
rem
   ...
   lf95 -c CFML_BVSpar.f90           -g  -chk   >> out
...
rem
   ...
   lf95 -c CFML_ILL_Instrm_data_LF.f90  -g  -chk   >> out
rem
   echo **---- Level 3 ----**
   echo .... Reflections, Atoms
rem
rem   lf95 -c CFML_Eos_Mod.f90          -g  -chk   >> out
...
rem
   lf95 -c CFML_Export_Vtk_LF95.f90  -g  -chk   >> out
...
rem
   ...
   if exist ..\Lahey_debug\LibW rmdir ..\Lahey_debug\LibW /S /Q
   mkdir ..\Lahey_debug\LibW
rem
   copy *.mod ..\Lahey_debug\LibW > nul
   move *.lib ..\Lahey_debug\LibW > nul
