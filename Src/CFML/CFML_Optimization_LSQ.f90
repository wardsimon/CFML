...
!!----
!!---- This file contains two modules: CFML_LSQ_Types and CFML_Optimization_LSQ.
!!----                                 --------------     ---------------------
...
!!----=======================================================================================================
!!----
!!---- MODULE: CFML_LSQ_Types
!!----   INFO: Module implementing derived types used in CFML_Optimization_LSQ.
!!----
!!---- HISTORY
!!----    Update: August - 2009
!!----
!!----    August - 2009 Created by JRC
!!---- DEPENDENCIES
...
!!
 ...
 Module CFML_Optimization_LSQ
    ...
    public  :: Marquardt_Fit, Levenberg_Marquardt_Fit, Final_Output
 ...
 Contains
    ...
    !!--++         use CFML_GlobalDeps, only: cp
    !!--++         use CFML_LSQ_Types,  only: LSQ_State_Vector_type
    !!--++         integer,                             intent(in) :: iv
    !!--++         real(kind=cp),                       intent(in) :: xv
    !!--++         real(kind=cp),                       intent(out):: ycalc
    ...
    !!----
    !!----
    !!----  This interface to lmder has been modified with respect to the original lmder1 (also available in
    !!----  the overloaded procedure) in order to use the LSQ types of this module.
    !!----  The residuals vector and the Jacobian matrix have been eliminated from the arguments in
    !!----  order to expose to the user only the important information that are stored in Vs of type
    !!----  LSQ_Vector_State_Type.
    ...
    !!----
    !!----      the value of iflag should not be changed by fcn unless
    !!----      the user wants to terminate execution of LM_Der.
    !!----      in this case set iflag to a negative integer.
    !!----
    !!----    m is a positive integer input variable set to the number of functions.
    !!----
    ...
    !!----
    !!----      the value of iflag should not be changed by fcn unless
    !!----      the user wants to terminate execution of LM_Dif.
    !!----      In this case set iflag to a negative integer.
    !!----
    !!----    m is a positive integer input variable set to the number of functions.
    !!----
    ...
    !!---- Subroutine Marquardt_Fit(Model_Functn, d, c, vs, Ipr, Chi2, scroll_lines)
    !!----
    !!---- The difference of these two procedures (v1 and v2) is only the way the model function
    !!---- is invoked and the different procedure for calculating handling the derivatives. The
    !!---- effective invoked procedure is selected depending whether the data are encapsulated (v2)
    !!---- or not (v1). The second version (v2) uses directly the State vector parameters for
    !!---- calculating the model. The user has not to handle directly the subset of refined parameters.
    ...
    Subroutine Marquardt_Fit_v1(Model_Functn,X,Y,W,Yc,Nobs,c,vs,Ipr,Chi2,scroll_lines)
       ...
       call Final_Output(chi2,FL,nobs,x,y,yc,w,Ipr,c,vs)
    ...
    Subroutine Marquardt_Fit_v2(Model_Functn,d,c,vs,Ipr,Chi2,scroll_lines)
       ...
       call Final_Output(chi2,FL,d%nobs,d%x,d%y,d%yc,d%sw,Ipr,c,vs)
    ...
    !!----
    !!----  Subroutine Final_Output(Chi2,Nobs,X,Y,Yc,W,Lun,c,vs,out_obscal)
    ...
    !!---- Update: August - 2009
    !!
    Subroutine Final_Output(Chi2,FL,Nobs,X,Y,Yc,W,Lun,c,vs,out_obscal)
       !---- Arguments ----!
       real(kind=cp),              intent(in)     :: chi2
