...
!!
 Module CFML_Magnetic_Structure_Factors
    ...
    private :: Calc_Table_MAB, Create_Table_mFR, Create_Table_mFR_Shub,  &
 ...
 Contains
    ...
    !!----   logical,                   optional,intent(in) :: nofpp
    !!---
    ...
    Subroutine Calc_Mag_Structure_Factor(n,hm,Cell,Grp,Atm,Stf,magonly,mdom,tdom,twin,nofpp)
      ...
      real(kind=cp), dimension(Atm%natoms)     :: otr, oti, frc, frs,snex
      real(kind=cp), dimension(3)              :: h, hnn, xi, cosa, side, aa, bb, t, car, ar, br
      Real(Kind=Cp), Dimension(3,Grp%Multip)   :: hr
      Real(Kind=Cp), Dimension(3,3)            :: sm,SMcos,SMsin
      !Real(Kind=Cp), Dimension(3,nats),save    :: cm
      logical                                  :: mag,nuc,mag_only
      character(len=1)                         :: tw
      real(kind=cp) :: ffr, ffi, ffx, cosr, sinr, scosr, z, ssinr, temp,snexi,x1
      real(kind=cp) :: rstar, x, yy, arg, arg2, exparg,ssnn
      ...
      !---------------------------        ipiof=i+iof
        ...
        if(nuc) then
           ...
           IF(nscat(n_pat) < 0 .AND. ni <= ABS(nscat(n_pat))) THEN
             rstar=SQRT(ssnn)*2000.0
             CALL sfint(n_pat,rstar,ni,ffr)
             ffi=dfpp(ni,n_pat)
             ffr=ffr+dfp(ni,n_pat)
           ELSE
             ffi=dfpp(ni,n_pat)                       ! Deltaf"
             ffr=dfp(ni,n_pat)+ac(9,ni,n_pat)         ! f0+Deltaf'
             DO ii=1,7,2
               ffr=ffr+ac(ii,ni,n_pat)*EXP(-ac(ii+1,ni,n_pat)*ssnn)
             END DO
           END IF
           if(present(nofpp)) ffi=0.0
        end if
        ffx=0.0
        mag=n_t(ipiof) == 1 .OR. n_t(ipiof) == 3  .and. Ihkl(nn,n_pat) /= 0

        IF(mag) Then
          ni=ptr(ipiof,2,n_pat)
          IF(nscat(n_pat) < 0 .AND. ni <= ABS(nscat(n_pat))) THEN
            CALL sfint(n_pat,rstar,ni,ffx)
          ELSE
            ffx=ac(7,ni,n_pat)
            DO ii=1,5,2
              ffx=ffx+ac(ii,ni,n_pat)*EXP(-ac(ii+1,ni,n_pat)*ssnn)  !Form factor value for Q=H+k
            END DO
          END IF
          snexi=pn*ffx*temp*xl(ipiof,5)     ! 0.26954.f(Q).Temp(i).Occ
        END IF
        ...
        SMcos=0.0; SMsin=0.0
       !+++++++++++++++++++++++++
        DO  ir=1,mSpG(iph)%Multip   !Loop over symmetry operators
       !+++++++++++++++++++++++++
          sm(:,:)=mSpg(iph)%Symop(ir)%Rot(:,:)
             t(:)=mSpG(iph)%Symop(ir)%tr(:)
          x=dot_product(t,hnn)
          h=matmul(hnn,sm)
          ...
          END IF
          IF(mag) Then
            SMcos(:,:)=SMcos(:,:)+cosr*mSpg(iph)%MSymop(ir)%Rot(:,:)
            SMsin(:,:)=SMsin(:,:)+sinr*mSpg(iph)%MSymop(ir)%Rot(:,:)
            rsai=mSpg(iph)%MSymop(ir)%Rot(:,:)
            if(present(mdom) .and. tw /= "T") rsai=matmul(rsai,real(mdom)) !modification for the current domain
          END IF
        !+++++++++++++++++++++++++
        ...
        IF(mag) Then
           ar = matmul(SMcos,cm(:,ipiof)/side)*side  !The introduction of Cell%cell
           br = matmul(SMsin,cm(:,ipiof)/side)*side  !of using non conventional settings for
           aa(:)= aa(:) + snexi*ar(:)
           bb(:)= bb(:) + snexi*br(:)
        END IF
      !---------------------------
      ...
      if( nuc ) then
        ...
        ! write(i_out,'(3i4,10F10.4)') nint(hnn(:)),a1,a2,a3,a4,b1,b2,b3,b4,av,bv
        asfa(nn,n_pat)=av !*flat(iph)
        bsfa(nn,n_pat)=bv !*flat(iph)
        ...
        fnn = a1*a1 + a3*a3 + b1*b1 + b3*b3

        !     FNN = av*av + bv*bv !For a single crystal

        ! FNN for single crystal is necessary for phase calculation in Fourier map
        ! (important for X-rays with strong anomalous scattering and non-centro S.G)

        !-----Calculate the phase in radians when CIF or Fourier file is required
        IF(jfou(n_pat) /= 0 .or. jcil== -1 .or. jcil==-4 .or. jcil == -5) THEN
          phasen(nn,n_pat)=atan2(bv,av)
          IF (phasen(nn,n_pat) < 0.0)  phasen(nn,n_pat) = phasen(nn,n_pat)+tpi
        END IF
        ! Modify FNN if single crystal data are used
        isingle=0
        IF(abs(irf(iph,n_pat)) == 4 .and. abs(ipow(iph,n_pat)) < 2 )THEN
          isingle=1
          fnn=av*av+bv*bv
        END IF
        !fnn=fnn*nlatti(iph)
      ...
      if(mag) then
        ...
        DO i=1,3
          ...
          END DO
        END DO
        fnn_mag=(x-0.25*yy/ssnn)!*nlatti(iph)      ! Halpern & Johnson F2= Fm.Fm* - (e.Fm)*(e.Fm)
        Mhv(:,nn,n_pat)=flat(iph)*cmplx(aa,bb)
      else
        fnn_mag=0.0      ! Halpern & Johnson F2= Fm.Fm* - (e.Fm)*(e.Fm)
        Mhv(:,nn,n_pat)=cmplx(0.0,0.0)
      end if
